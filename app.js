...
(function () {
  ...
  function ensureRail() {
    ...
    nav.innerHTML =
      ...
      '<div class="rail-scroll">' +
        ...
        modesTop +
        scopesHtml +
        '<div class="rail-group-label rail-sec-toggle" id="rail-views-label" style="display:none"><span class="rail-sec-caret" id="rail-views-caret">▸</span>Views<span class="rail-sec-count" id="rail-views-count"></span></div>' +
  ...
  function renderScopes() {
    ...
    var scopesHtml = vis.map(function (si) {
      ...
      if (s.kind === 'buttons') {   // segmented toggle inside the scopes group
        ...
        // Searchable single-select: a text input + native <datalist> (type to
        // filter). The input shows the current selection's LABEL; on change we
        // map the typed label back to its value (blank clears).
        var curV = (s.active && s.active()) || '';
        var curLabel = '';
        opts.forEach(function (o) { if (String(o.value) === String(curV)) curLabel = o.label; });
        // The × sits INSIDE a relative wrapper rather than after the input, so
        // it stays pinned to the right edge whatever the rail is dragged to.
        // Rendered only when there is something to clear.
        body = '<span class="rail-search-wrap">'
          + '<input class="rail-scope-search" data-scope="' + si + '" list="rail-scope-dl-' + si + '"'
          + ' placeholder="' + esc(s.placeholder || ('All ' + String(s.label).toLowerCase())) + '"'
          + ' value="' + esc(curLabel) + '" autocomplete="off">'
          + (curLabel ? '<button type="button" class="rail-search-x" data-clear-scope="' + si
                        + '" title="Clear" aria-label="Clear">\u00d7</button>' : '')
          + '</span>'
          + '<datalist id="rail-scope-dl-' + si + '">'
          + opts.map(function (o) { return '<option value="' + esc(o.label) + '"></option>'; }).join('')
          + '</datalist>';
      } else {
    ...
    box.querySelectorAll('.rail-scope-search[data-scope]').forEach(function (inp) {
      var commit = function () {
        var s = CFG.scopes[+inp.dataset.scope];
        if (!s || !s.onSelect) return;
        var typed = (inp.value || '').trim().toLowerCase();
        var val = '';
        ((s.options && s.options()) || []).forEach(function (o) {
          if (String(o.label).toLowerCase() === typed) val = o.value;
        });
        s.onSelect(val);   // no match / blank → clears
        refresh();
      };
      inp.addEventListener('change', commit);
    });
    box.querySelectorAll('.rail-search-x[data-clear-scope]').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        e.stopPropagation();          // the row toggles its group on click
        var s = CFG.scopes[+btn.dataset.clearScope];
        if (s && s.onSelect) { s.onSelect(''); refresh(); }
      });
    });
  ...
  function renderViews() {
    ...
    var list = (V && V.list) ? (V.list() || []) : [];
    // Active-view badge beside the VIEWS heading — mirrors the Filters (N) badge.
    // Set it here, before the collapsed/hidden early return, so it stays visible
    // when the section is collapsed (a re-applied saved view must never read as
    // "no filter"). Blank in the default state, just like the Filters badge.
    var vcount = document.getElementById('rail-views-count');
    if (vcount) {
      var summ = '';
      if (show) {
        if (typeof V.activeSummary === 'function') {
          // OPT-IN, robust: the tracker computes the badge text itself. Needed
          // where list() does NOT prepend a default system view first (e.g. the
          // student tracker, whose list is just the tab's presets) — there the
          // heuristic below would wrongly blank when the active view is list[0].
          try { summ = V.activeSummary() || ''; } catch (_) { summ = ''; }
        } else {
          // Fallback heuristic: show the active view's NAME unless it's the
          // default (the "All …" system view trackers prepend first, which is
          // active in list[0] in the default state — so the badge is blank then).
          var act = list.find(function (v) { return v.active; });
          var isDefault = !!(list[0] && list[0].active);
          if (act && !isDefault) summ = act.name;
        }
      }
      if (summ) {
        var canClear = !!(V && typeof V.onClear === 'function');
        vcount.innerHTML = '  ·  ' + esc(summ) +
          (canClear ? '<span class="rail-views-x" title="Clear view">&times;</span>' : '');
        if (canClear) {
          var vx = vcount.querySelector('.rail-views-x');
          if (vx) vx.addEventListener('click', function (e) {
            e.stopPropagation(); try { V.onClear(); } catch (_) {}
          });
        }
      } else {
        vcount.textContent = '';
    ...
    // `infoHtml` is OPT-IN: a tracker returns markup (an info bubble, say) to sit
    // beside the view name. Trackers that do not supply it render exactly as
    // before — this must stay true, the shell is shared with live dashboards.
    var infoHtml = (typeof V.infoHtml === 'function') ? V.infoHtml : null;
    box.innerHTML = list.map(function (v, i) {
      var extra = '';
      if (infoHtml) { try { extra = infoHtml(v) || ''; } catch (_) { extra = ''; } }
      // With a bubble the name goes in its own span: .rail-view clips with
      // overflow:hidden, so a long name pushed the bubble past the edge and it
      // vanished. Flex + a truncating label keeps the bubble on screen.
      var label = extra ? '<span class="rail-view-label">' + esc(v.name) + '</span>' : esc(v.name);
      // The native tooltip carries the description when there is one. A rendered
      // bubble is easy to miss at this size and gets clipped by a long name;
      // hovering the row itself always works, needs no styling, and is what a
      // user tries first.
      var tip = v.desc ? (v.name + ' — ' + v.desc) : v.name;
      return '<button class="rail-view' + (v.active ? ' active' : '') + (extra ? ' has-info' : '') + '" data-view="' + i + '" title="' + esc(tip) + '">' + label + extra + '</button>';
  ...
  function renderFilters() {
    ...
    html += addOpen
      ? '<div class="rail-addfilter"><span class="rail-search-wrap">'
        + '<input id="rail-addfilter-search" class="rail-addfilter-search" placeholder="Search fields…" autocomplete="off">'
        + '<button type="button" class="rail-search-x" id="rail-addfilter-x" title="Clear" aria-label="Clear" hidden>\u00d7</button></span>' +
    ...
    if (addOpen) {
      var inp = document.getElementById('rail-addfilter-search');
      var x = document.getElementById('rail-addfilter-x');
      if (inp) {
        inp.focus();
        if (x) {
          var sync = function () { x.hidden = !inp.value; };
          inp.addEventListener('input', sync);
          x.addEventListener('click', function (e) {
            e.stopPropagation();
            inp.value = '';
            sync();
            inp.dispatchEvent(new Event('input'));   // re-filter the list
            inp.focus();
          });
          sync();
        }
...
async function load() {
  ...
  let _dqv = null, _dqs = null, _dqt = null;
  try { const _q = new URLSearchParams(location.search); _dqv = _q.get('view'); _dqs = _q.get('search'); _dqt = _q.get('term'); } catch(_){}
  ...
  // Deep-links, applied AFTER any view restore (applyView overwrites these).
  // The Program Tracker's Courses tab links out with ?search=<course code>&term=all.
  //  ?search=<course code> pre-fills the header search (course-code match among
  //    other fields). Does NOT pin the term on its own.
  //  ?term=all opens with ALL terms (overrides the Fall 2026 default) so a course
  //    not offered in Fall still shows its Spring/Summer sections on arrival; the
  //    owner then narrows the term. Only the literal 'all' is supported.
  const _termAll = _dqt && _dqt.toLowerCase() === 'all';
  if (_termAll) filters.term = [];   // [] = all terms
  if (_dqs) { filters.search = _dqs; const se = $('#f-search'); if (se) se.value = _dqs; _syncSearchX(); }
  if (_termAll || _dqs) renderAll();  // reflect the term buttons + table
}
...
// URL to a course in the Portfolio Tracker's Courses view (all courses), filtered
// to this code. Environment-aware: published Section site → published Portfolio;
// local → local :5001. The Portfolio tracker reads ?app=portfolio + view=
// portfolio_courses; the ?search= filter is honored once its inbound deep-link
// ships (until then it opens the Courses view unfiltered). If the Portfolio chat
// finalizes a different URL/param, change it HERE (one place).
function _portfolioCourseUrl(code){
  const base = window._staticMode ? 'https://wmeleis.github.io/program-tracker/' : 'http://localhost:5001/';
  return base + '?app=portfolio&view=portfolio_courses&search=' + encodeURIComponent(code || '');
}
function renderDetail(s){
  ...
  // Link out to this course in the Portfolio Tracker's Courses view (mirror of the
  // Program→Section link). Degrades gracefully: until the Portfolio tracker reads
  // ?search=, the link just opens its Courses view unfiltered.
  if (s.course_code) {
    const pl = el('div', 'sd-portfolio-row');
    pl.innerHTML = '<a class="sd-portfolio-link" href="' + _portfolioCourseUrl(s.course_code) +
      '" target="_blank" rel="noopener" title="Open this course in the Portfolio Tracker">' +
      'Course portfolio ↗</a>';
    d.appendChild(pl);
  }
...
function renderViewTiles(){
  ...
  bar.innerHTML = label + tileViews.map(v=>{
    ...
    // Superscript × to clear the active view (not on the "All sections" tile — it IS the cleared state).
    const clearX = (active && v.id!=='all') ? `<sup class="pv-tile-x" onclick="event.stopPropagation();clearActiveView()" title="Clear view">×</sup>` : '';
    return `<button class="pv-tile${active?' active':''}" onclick="applyView('${v.id}')" title="${esc(v.name)}">
      ...
      <span class="pv-tile-label">${esc(v.name)}${info}${clearX}</span></button>`;
  }).join('');
}

// Clear the active saved view → back to the default "All sections" (no filter) state.
function clearActiveView(){ applyView('all'); }
...
function bindControls(){
  ...
  $('#f-search').oninput=e=>{filters.search=e.target.value;_syncSearchX();renderTable();};
  document.addEventListener('click', e=>{ if(!document.contains(e.target)) return; if(!e.target.closest('.ms')) closeAllMsPanels(); });
  _syncSearchX();
}
// Show the search clear-× only when there's text; clear resets the search filter.
function _syncSearchX(){ const x=$('#search-x'); if(x) x.hidden = !filters.search; }
window.clearSearch=()=>{ filters.search=''; const se=$('#f-search'); if(se) se.value=''; _syncSearchX(); renderTable(); if(se) se.focus(); };
...
function _sectionRailFields(){
  ...
  return [
    // Level / College / Campus / Modality are permanent Focus scopes (_sectionScopes),
    // so they're intentionally NOT in this scalable "+ Add filter" list — otherwise a
    // Focus selection would also surface a duplicate filter chip (matches the program
    // tracker, which excludes its scope filters here too).
    // Course
...
// "Focus" scopes — the always-visible primary pickers at the top of the rail
// (Level buttons + College / Campus dropdowns), like the program tracker. Single-
// select: College/Campus write a 1-element array (or null=all) so the existing
// multi-select read paths in baseFiltered are unchanged; _one collapses a multi
// selection to its lone value (blank when 0 or >1, so the dropdown reads "All").
function _sectionScopes(){
  const _one = arr => { const a=(arr||[]).filter(Boolean); return a.length===1 ? a[0] : ''; };
  return [
    { kind:'buttons', label:'Level',
      options: () => [{value:'UG', label:'Undergrad'}, {value:'GR', label:'Graduate'}],
      active: () => filters.level,
      onSelect: v => { filters.level = (filters.level===v ? '' : v); renderAll(); } },
    { label:'College', placeholder:'All colleges',
      options: () => msValues('college').map(c => ({value:c, label:c})),
      active: () => _one(filters.college),
      onSelect: v => { filters.college = v ? [v] : null; renderAll(); } },
    { label:'Campus', placeholder:'All campuses',
      options: () => msValues('campus').map(c => ({value:c, label:c})),
      active: () => _one(filters.campus),
      onSelect: v => { filters.campus = v ? [v] : null; renderAll(); } },
    { label:'Modality', placeholder:'All modalities',
      options: () => msValues('modality').map(v => ({value:v, label:modShort(v)})),   // MOD_ORDER via msValues
      active: () => _one(filters.modality),
      onSelect: v => { filters.modality = v ? [v] : null; renderAll(); } },
  ];
}
...
if (typeof initTrackerShell === 'function') initTrackerShell({
  ...
  scopesLabel: 'Focus',
  scopes: _sectionScopes(),   // Level buttons + College / Campus dropdowns
  tools: [ { label: 'Console', onClick: () => openConsoleModal() } ],
  views: {
    enabled: () => true,
    list: () => [SECTION_ALL_VIEW, ...getTeamViews(), ...getPersonalViews()]
                 .map(v => ({ id:v.id, name:v.name,
                   active: v.id==='all' ? (!activeViewId || activeViewId==='all') : v.id===activeViewId })),
    // Clear-× on the ACTIVE-view BADGE (the blue "· <name>" beside the VIEWS
    // header) via the shell's opt-in onClear slot — mirrors the per-filter clear-×.
    // The shell only renders the × when onClear is set AND a view is active, so
    // "All sections" (the cleared state, blank badge) shows none.
    onClear: () => clearActiveView(),
    // Our list() prepends no no-filter default view, so the rail's fallback badge
    // heuristic (which blanks when list[0] is active) would wrongly blank on the
    // first team view. Report the active view name directly — blank in the
    // default/"all" state (activeViewId null|'all' matches no team/personal view).
    activeSummary: () => { const v=[...getTeamViews(), ...getPersonalViews()].find(x=>x.id===activeViewId); return v?v.name:''; },
...
window.clearActiveView=clearActiveView;
